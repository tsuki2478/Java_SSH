	    ...
	    <tr>
            ...
            <td>
                <ul>
                	<!-- 取出所有的一级菜单 
                		给之前设置的菜单 添加默认选项
                		循环出来的菜单是否在对应的中间表中，在选中，不在不选中
                	-->
                  <s:iterator value="menusList.{? #this.pid == 0}" var="l1">
                  	 <li>
						<label class='checkbox inline'>
						<input type='checkbox' name='menus.id'  
						value='<s:property value="#l1.id"/>' 
							<s:property value="#l1.id in menus.{id}?'checked':''"/>
						/>
						...
						<ul>
							...
							<s:iterator value="menusList.{? #this.pid ==#l1.id}" var="l2">
								<li>
									...
									<s:property value="#l2.id in menus.{id}?'checked':''"/>/>
									...
									<ul>
										...
										<s:iterator var="l3" value="menusList.{? #this.pid == #l2.id}">
											<li>
												...
												<s:property value="#l3.id in menus.{id}?'checked':''"/> />
